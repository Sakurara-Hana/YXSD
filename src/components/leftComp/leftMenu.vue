<template>
  <div class="leftMenu">
    <label v-for="leftMenu in leftMenus" :key="leftMenu.id" @click="getRef">
      <!--只有一级菜单-->
      <el-menu-item
        v-if="leftMenu.child.length==0&&leftMenu"
      ...
      <!--有二级菜单-->
      <el-submenu
        v-if="leftMenu.child.length!=0&&leftMenu.child"
      ...
      >
       ...
       <!-- :key="leftMenu.id" -->
        ...
        <el-menu-item
          class="cli"
          v-for="clist in leftMenu.child"
          :key="clist.id"
          :index="clist.id.toString()"
